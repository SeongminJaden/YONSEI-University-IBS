...
| **모션 모드** | 미리 정의된 손 제스처를 버튼으로 제어 | 시연/테스트 |
...
│  └────────────────────────────────────────────────────────────────────┘ │
│                                                                          │
│  ┌─ 모션 제스처 (모션 모드 선택 시 표시) ─────────────────────────────┐ │
│  │  [주먹] [펴기] [V] [엄지척] [OK] [1] [2] [3] [4] [5]               │ │
│  └────────────────────────────────────────────────────────────────────┘ │
│                                                                          │
│  ┌─ 손가락 상태 ──────────────────────────────────────────────────────┐ │
...
### 모드 3: 모션 모드 (Motion Mode)

미리 정의된 손 제스처를 버튼 클릭으로 즉시 실행합니다.
...
3. **제스처 실행**
   - 원하는 제스처 버튼 클릭
   - Arduino가 자동으로 연결되고 각도 전송

#### 지원 제스처

| 제스처 | 버튼 | 각도값 [Thumb, Index, Middle, Ring, Pinky] | 설명 |
|--------|------|-------------------------------------------|------|
| 주먹 | `주먹` | [180, 180, 180, 180, 180] | 모든 손가락 굽힘 |
| 손 펴기 | `펴기` | [0, 0, 0, 0, 0] | 모든 손가락 펴기 |
| V 사인 | `V` | [180, 0, 0, 180, 180] | 검지, 중지만 펴기 |
| 엄지 척 | `엄지척` | [0, 180, 180, 180, 180] | 엄지만 펴기 |
| OK 사인 | `OK` | [60, 60, 0, 0, 0] | 엄지+검지 OK 모양 |
| 숫자 1 | `1` | [180, 0, 180, 180, 180] | 검지만 펴기 |
| 숫자 2 | `2` | [180, 0, 0, 180, 180] | 검지, 중지 펴기 |
| 숫자 3 | `3` | [180, 0, 0, 0, 180] | 검지, 중지, 약지 펴기 |
| 숫자 4 | `4` | [180, 0, 0, 0, 0] | 엄지만 굽힘 |
| 숫자 5 | `5` | [0, 0, 0, 0, 0] | 모든 손가락 펴기 |

> **참고**: 각도값 0 = 손가락 펴기, 180 = 손가락 굽히기
